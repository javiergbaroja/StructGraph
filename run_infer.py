...
options:
   ...
   --inst_dir=<path>    Path to prediction directory..
...
usage:
    wsi (--input_dir=<path>) (--output_dir=<path>) [--proc_mag=<n>]\
        [--cache_path=<path>] [--input_mask_dir=<path>] [--inst_pred_dir=<path>] \
...
options:
    --input_dir=<path>      Path to input data directory. Assumes the files are not nested within directory.
    --output_dir=<path>     Path to output directory.
    --inst_pred_dir=<path>     Path to instance prediction map directory.
...
if __name__ == '__main__':
    ...
    if sub_cmd == 'wsi':
        run_args.update({
            'input_dir'      : sub_args['input_dir'],
            'output_dir'     : sub_args['output_dir'],
            'inst_pred_dir': sub_args['inst_pred_dir'],
    ...
    if sub_cmd == 'tile':
        ...
        infer = InferManager('tile',**method_args)
        infer.process_file_list(run_args)
    else:
        from infer.wsi import InferManager
        infer = InferManager('wsi',**method_args)
