...
class SeNuclsNet(Net):
    ...
    def __init__(self, input_ch=3, nr_types=None, freeze=False, mode='original'):
        ...
        self.textrcnn = RNN(embedding_dim=64, hidden_dim=128, output_size=64)
        self.shapercnn = RNN(embedding_dim=67, hidden_dim=128, output_size=64)
        self.classifier = nn.Conv2d(64, nr_types-1, 1, stride=1, padding=0, bias=True)
    ...
    def forward(self, imgs, bboxes,centers,edge_points,edge_indexes,batch_pos_emb,batch_select_shape_feats,mode):
        ...
        for branch_name, branch_desc in self.decoder.items():
            ...
            #print(pred_class.shape)
                ...
                node_feature = []
                batch_feature = []
                batch_length = [len(center) for center in centers]
                for i in range(len(centers)):
                    ...
                    #print(single_edge_indexs.shape)
                    ...
                    if i > 0:
                        node_feature = torch.cat((inst_features[np.sum(batch_length[0:i]):np.sum(batch_length[0:i])+batch_length[i],:],aligned_center_feature,aligned_shape_feature,single_pos_emb),1)
                    else:
                        node_feature = torch.cat((inst_features[0:batch_length[i],:],aligned_center_feature,aligned_shape_feature,single_pos_emb),1)
            ...
            #print(inst_features.shape)
                pred_class = self.classifier(batch_feature)
            #print(pred_class.shape)
            ...
            #print(u3.shape, u2.shape, u1.shape, u0.shape)
                ...
                for i in range(len(centers)):
                    ...
                    node_feature = torch.cat((inst_features,aligned_center_feature,aligned_shape_feature,single_pos_emb),1)
                    #print(node_feature.shape)
                    updated_feature = self.deepgcn(node_feature,single_edge_indexs,single_edge_points)


                    batch_feature = updated_feature.reshape(updated_feature.shape[0],updated_feature.shape[1],1,1)
